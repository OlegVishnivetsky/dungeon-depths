...
public static class AStar
{
    ...
    private static Node FindShortestPath(Node startNode, Node targetNode, GridNodes gridNodes, List<Node> openNodeList, HashSet<Node> closeNodeHashSet,
        ...
        HashSet<Node> closeNodeHashSet, InstantiatedRoom instantiatedRoom)
    {
        ...
        for (int i = -1; i <= 1; i++)
        {
            for (int j = -1; j <= 1; j++)
            {
                ...
                validNeighbourNode = GetValidNodeNeighbour(currentNodeGridPosition.x + i, currentNodeGridPosition.y + j, gridNodes, closeNodeHashSet, instantiatedRoom);
    ...
    private static Node GetValidNodeNeighbour(int neighnourNodeXPosition, int neighnourNodeYPosition, GridNodes gridNodes,
        HashSet<Node> closeNodeHashSet, InstantiatedRoom instantiatedRoom)
    {
        if (neighnourNodeXPosition >= instantiatedRoom.room.templateUpperBounds.x - instantiatedRoom.room.templateLowerBounds.x || neighnourNodeXPosition < 0
            || neighnourNodeYPosition >= instantiatedRoom.room.templateUpperBounds.y - instantiatedRoom.room.templateLowerBounds.y || neighnourNodeYPosition < 0)
        {
            return null;
        }

        Node neighbourNode = gridNodes.GetGridNode(neighnourNodeXPosition, neighnourNodeYPosition);

        int movementPenaltyForGridSpace = instantiatedRoom.aStarMovementPenalty[neighnourNodeXPosition, neighnourNodeYPosition];

        if (closeNodeHashSet.Contains(neighbourNode) || movementPenaltyForGridSpace == 0)
        {
            return null;
        }
        else
